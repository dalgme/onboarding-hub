...
- **미진행 리마인드** — `src/lib/reminders.ts`. 의뢰인 다음 단계가 3일째 그대로면 1차,
  7일째면 2차(화면공유 제안) 문구를 「보낼 카톡」에 올린다. 평일 10~18시 KST에만 만들고,
  프로젝트당 하루 1건·단계당 2회. 기준 시각 `stallSince` = 접속 안내를 보낸 시각 · 직전 의뢰인
  단계의 확인·건너뜀 · 의뢰인의 마지막 완료 요청·코멘트 · 보류 해제 중 가장 늦은 것.
  정지 조건(`reminderStopReason`, `todo.ts`)은 **문구를 만들 때와 카드를 거둘 때가 같은 함수를
  읽는다** — onboarding이 아님 · assisted · 보류 중 · 막힘 · 내 쪽 오류 · 내 차례 · 최근 7일
  의뢰인 코멘트. 거둠에는 「카드 생성 뒤 의뢰인 접속」이 더해진다(작성 조건에는 넣지 않는다 —
  들어와서 방치하는 경우가 리마인드의 대상이다). 결제가 필요한 단계는 문구 대신 칩
- **상태 자동 전이** — `src/lib/lifecycle.ts`. 첫 제작자 단계 앞의 의뢰인 단계가 전부
  확인·건너뜀이고 범위가 확정되면 `building`, 「배포 및 인수인계」가 확인되면 `delivered`.
  사건 핸들러에서만 부르고 앞으로만 간다 — tick에 두면 사람이 설정 탭에서 되돌린 것과 싸운다
...
- **Claude 서비스 계정 (`anthropic-service-account`)** — 5번째 단계, 4번째와 함께 건너뜀
  (`SKIP_TOGETHER`). **개인 키는 내가 조직에서 나가는 순간 보관 처리된다** — 운영 키는 반드시
  의뢰인(Admin)이 Settings → Service accounts 에서 만든 서비스 계정 명의로 발급한다(기본
  워크스페이스에는 자동 포함, 헤더 불필요). 개발은 내 개인 키(`*-dev`)로 시작해도 되지만 운영 배포
  전에 서비스 계정 키로 바꾼다. 확인은 2탭(「보임/안 보임」 — 내 열쇠 발급 창의 「연결된 계정」).
  (실제 사고 09-17: 이 단계가 없어 요청이 관리자 머릿속에만 있었다)
...
2. 운영 Anthropic 키가 「서비스 계정」 키인지 확인 — 내 개인 키면 교체·재배포  ← 탈퇴 전
3. 발급받은 토큰·키 폐기 (GitHub PAT / Vercel / 내 개발용 Anthropic 키 등) ← 먼저
   (서비스가 실제로 쓰는 서비스 계정 키는 의뢰인 조직 소유라 남긴다)
4. 연결된 모든 조직에서 내 멤버 권한 탈퇴                    ← 나중
   (GitHub·Vercel·Supabase + 연결했다면 Anthropic·Resend·Solapi)
5. 로컬 클론 · .env · 덤프 파일 삭제
6. 의뢰인 포털 접근 회수 (`project_guests` 행 삭제 + Auth 사용자 삭제)
7. 의뢰인에게 완료 안내 + 멤버 목록 확인 방법 안내
```

> 비밀번호에는 만료가 없다. 6번을 건너뛰면 종료된 프로젝트의 의뢰인이
> 포털·도우미·검증 API를 계속 호출할 수 있다.

> **2·3번이 4번보다 먼저다.** 멤버를 먼저 지우면 조직에서 나온 뒤라
> 남아 있는 토큰을 회수할 방법이 사라지고, 개인 키로 돌던 운영 AI 기능은 그날 멈춘다.
