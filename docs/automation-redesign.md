...
| 4 | 접속 안내문 작성·카톡 전송 | 손 | 위저드가 안내문(주소+이메일+임시 비밀번호+보조 링크)을 만들어 「보낼 카톡」 카드로 띄운다. 나는 [복사] → 카톡 → [보냈음]. 「보냈음」 시각 = `access_sent_at`. 비밀번호는 발급 화면에만 있고 장부(`notices.body`)에는 남기지 않는다. 메일 병행 없음(3판) |
...
| 20 | 「보낼 카톡」 처리 (신설·수동, 건당 30초) | — | `/a` 최상단 「보낼 카톡 N건」 카드에서 [복사] → 카톡 붙여넣기 → [보냈음]. 4시간 넘게 남아 있으면 푸시 1회, 09:00 요약에 건수. 근거 조건이 사라진 문구는 tick이 자동으로 거둔다(카드에서 내림) |
...
| `support_tier='assisted'` | 리마인드 없음. 첫 화면부터 화면공유 예약 제안 | – | 첫 접속 24h 내 시간이 안 남겨지면 「전화로 일정 잡기」 |
...
**거둠(신설)**: pending 행의 근거 조건이 사라지면(단계 verified·의뢰인 로그인·의뢰인 코멘트 도착·프로젝트 closed·관리자가 상태를 바꿈) tick이 `skipped(condition_cleared)`로 바꾸고 카드에서 내린다. 관리자가 철 지난 문구를 보내는 일을 코드로 막는다.
...
- 동작: [복사] = `navigator.clipboard.writeText`(실패 시 textarea 선택 폴백) · [보냈음] → `sent`+`sent_at` — credentials면 `projects.access_sent_at`도 · [보내지 않음] → `skipped(admin)`. 세 버튼 모두 서버 액션(service_role). 의뢰인 세션은 이 테이블을 읽지도 쓰지도 못한다
- 푸시: outbox 행 생성 시 「보낼 카톡 1건 · {의뢰인}: {사유}」(`/a#outbox`로 열림). 4시간 넘게 pending이면 하루 1회 더(`outbox_stale` 키). 09:00 요약에 건수
- 거둠·대체: §4-4(조건 소멸 → `skipped(condition_cleared)`, 같은 주제 새 문구 → 이전 행 `superseded`). 카드에는 주제당 최신 1건만
- 템플릿: `ko.ts` `outbox.*` 9종(CX 원칙 §4-4). 카톡 1건 ≤ 8줄·300자·링크 ≤ 1개(로그인 링크는 M14 프래그먼트 — 카톡 미리보기 스크래퍼도 프래그먼트는 받지 않는다, 확인 필요 10-4). 첫 줄 「{의뢰인 이름}님, 」+ 한 일 인정, 마지막 줄 「이 카톡으로 답 주셔도 됩니다」 또는 「화면공유 20분이면 함께 끝낼 수 있어요」
