...
| 59 | (실제 운영 사고 09-14) Supabase 가 자동 확인되면서 「다음은 Claude 입니다」 카드가 만들어졌는데, 다음 tick 이 그 자리에서 거둬 관리자가 보지도 못했다 | **반영** | `next_step` 거둠 술어가 가리킨 단계의 `todo` 만 「아직 필요함」으로 보던 것을 고쳤다. `returned`(한 가지만 더)는 「의뢰인이 스스로 시작했다」가 아니라 「고쳐 달라고 기다리는 중」이라 안내가 더 필요하다. `doing`·`blocked`은 그대로 거둔다 |
| 60 | (실제 운영 사고 09-17) Anthropic 운영 키의 명의 요건(서비스 계정)이 온보딩 단계에 없어 요청이 관리자 머릿속에만 있었다 — 개발 시작 시 개인 키(`sangrok-dev`)만 있었고, 개인 키는 관리자가 조직을 나가면 보관 처리된다 | **반영** | 의뢰인 단계 「Claude(AI) 서비스 계정 만들기」(`anthropic-service-account`)를 4번째 뒤에 신설. 2탭은 「보임/안 보임」(내 열쇠 발급 창의 「연결된 계정」) — 단계별 문구 덮어쓰기(`verifyCodeByKey`·`ackByKey`, `src/lib/verify/copy.ts`)로 초대 문장을 쓰지 않는다. 「안 보임」은 서비스 계정 화면을 가리키는 재요청 문구. Claude 초대를 건너뛰면 같이 건너뛴다(`SKIP_TOGETHER`). 종료 체크리스트에 「운영 키가 서비스 계정 키인지 확인」을 토큰 폐기 앞에 추가. 같은 검수에서 잡힌 두 가지도 함께: 포털 「다음 할 일」이 앞쪽 `todo` 보다 `returned` 를 먼저 보여준다(§5 「최상단」을 코드로) · `next_step` 카드가 그 단계의 재요청·리마인드 카드를 즉시 대체한다(「확인 부탁」과 「확인됐습니다」가 다음 tick 까지 나란히 서 있지 않게) |
