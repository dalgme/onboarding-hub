...
4. **⚠ 가장 중요합니다** — 같은 Billing 화면에서 **자동 충전(Auto reload)을 반드시 켜 주세요.** "잔액이 $5 아래로 내려가면 $20 충전"처럼 설정합니다. 이걸 꺼두면 크레딧이 떨어지는 순간 **AI 기능이 멈추고, 겉으로는 서비스 고장처럼 보입니다.** 금액은 원하는 대로 조정하셔도 되지만, 켜는 것 자체는 필수입니다.
...
// 자동 확인이 없는 의뢰인 단계의 「완료했습니다」 전 자기확인 목록.
// 이 단계들은 API로 확인할 수 없으니, 의뢰인이 스스로 눈으로 확인하게 한다.
// (실제 사고: Claude 단계에서 초대를 보내지 않은 채 「완료」가 눌렸다)
// step.key 기준 — DB에 이미 복사된 단계에도 적용된다. 저장하지 않는다.
export const DONE_CHECKLIST: Record<string, readonly string[]> = {
  "connect-anthropic": [
  ...
  "scope-review": [
    ...
    "이야기한 것과 다르거나 빠진 내용은 질문·요청에 남겼습니다 (없으면 그대로 완료)",
  ],
  domain: [
    "도메인 보유 여부(구입처, 또는 원하는 주소)를 질문·요청에 남겼습니다",
  ],
};
